...
//         mutex_lock(&vif->wdev.mtx); /* PATCHED: wdev.mtx removed in 6.x */
//         __acquire(&vif->wdev.mtx); /* PATCHED: wdev.mtx removed in 6.x */
...
//         mutex_unlock(&vif->wdev.mtx); /* PATCHED: wdev.mtx removed in 6.x */
//         __release(&vif->wdev.mtx); /* PATCHED: wdev.mtx removed in 6.x */
    }
    rwnx_del_csa(vif);
}
#endif
                                               ...
                                               struct vif_params *params)
{
    ...
    if (type == NL80211_IFTYPE_AP_VLAN) {
        ...
        ether_addr_copy(vif->wdev.address, params->macaddr);
    } else {
        u8 mac_addr[ETH_ALEN];

        ether_addr_copy(mac_addr, rwnx_hw->wiphy->perm_addr);
        mac_addr[ETH_ALEN - 1] ^= vif_idx;
                                        ...
                                        )
{
    ...
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 0, 0))
    /* Driver does not support MLO. cfg80211 passes 0 for non-MLO operation. */
    (void)link_id;
    #endif
...
)
{
    ...
    return rwnx_send_mesh_path_update_req(rwnx_hw, rwnx_vif, (void *)dst, (void *)next_hop, &cfm);
}
...
)
{
    ...
    return rwnx_send_mesh_path_update_req(rwnx_hw, rwnx_vif, (void *)dst, NULL, &cfm);
}
...
//     .change_beacon = rwnx_cfg80211_change_beacon, /* PATCHED: signature changed in 6.x */
...
//     .set_tx_power = rwnx_cfg80211_set_tx_power, /* PATCHED: signature changed in 6.x */
