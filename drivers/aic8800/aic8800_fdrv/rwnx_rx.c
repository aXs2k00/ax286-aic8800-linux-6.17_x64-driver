                                 ...
                                 u32 vend_it_present)
{
    ...
    // Check for HE frames
    if (rxvect->format_mod == FORMATMOD_HE_SU) {
        /* Zero-init to avoid leaking stack bytes in radiotap header. */
        ...
        /*
         * Use skb->data as base to avoid false-positive fortify warnings from
         * variable-length radiotap buffers.
         */
        {
            size_t he_off = pos - (u8 *)rtap;

            if (WARN_ONCE(he_off + sizeof(he) > rtap_len,
                          "radiotap overflow (off=%zu len=%d he=%zu)\n",
                          he_off, rtap_len, sizeof(he)))
                return;

            memcpy(skb->data + he_off, &he, sizeof(he));
        }
        pos += sizeof(he);
    }
